...
class CropPredictionSerializer(serializers.ModelSerializer):
    class Meta:
        model = CropPrediction
        fields = "__all__"
        read_only_fields = ["user", "predicted_crop", "created_at"]

    def validate_N(self, value):
        if value < 0 or value > 150:
            raise serializers.ValidationError(
                "Nitrogen must be between 0 and 150"
            )
        return value
    ...
    def validate_P(self, value):
        if value < 0 or value > 150:
            raise serializers.ValidationError(
                "Phosphorus must be between 0 and 150"
            )
        return value
    ...
    def validate_K(self, value):
        if value < 0 or value > 210:
            raise serializers.ValidationError(
                "Potassium must be between 0 and 210"
            )
        return value
